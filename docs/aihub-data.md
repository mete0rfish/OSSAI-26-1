...
학습자가 [AIHub 멀티모달 정보검색 데이터](https://aihub.or.kr/aihubdata/data/view.do?aihubDataSe=data&currMenu=&dataSetSn=71813&topMenu=)에서
이용 조건을 확인하고 `멀티모달 정보검색 데이터_Sample`을 내려받는다. 이 저장소는
AIHub 원본과 라벨을 재배포하지 않는다.
...
질문은 `data/cases/week-01-aihub.yaml`에 있으며 사람이 읽고 수정한다.
...
현재 자동 점검 결과는 정답 30건 일치, 답변 보류 4건, 페이지 라벨 불일치 0건이다.
표·복합 날짜 6건은 텍스트 계층으로 확인할 수 없다. 이 6건과 답변 보류
4건은 `reports/week-01/eda.json`의 `label_text_checks`에서 `sample_id`를 찾아 원본
페이지 이미지와 직접 대조한다. 이 저장소는 수동 검토 완료 서명을 자동 gate로
구현하지 않았으므로, 자동 점검 통과만으로 라벨 정확성이 증명되지는 않는다.
